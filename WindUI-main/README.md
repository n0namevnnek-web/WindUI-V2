...
When a window has a `Folder`, WindUI initializes `Window.SaveManager` and `WindUI.SaveManager`. It stores simple key/value settings in `WindUI/<Folder>/settings.json`:

```luau
local SaveManager = Window.SaveManager

SaveManager:SaveValue("selectedMode", "Fast")
local selectedMode = SaveManager:LoadValue("selectedMode", "Safe")
```

`SaveValue` writes immediately. `SaveManager:Save()` and `SaveManager:Load()` are also available for explicit control.
